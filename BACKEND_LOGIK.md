...
**Grenze:** A08 trennt unabhängige sichere und unsichere Mutationsteile; das Beantworten und Fortsetzen übernimmt der inzwischen geschlossene W05-Pfad. Der allgemeine Recovery-/Nachtlauf über Capture-, Chat- und Promotionsfehler (A11/W06) ist seit 2026-09-12 vollständig geschlossen. Archivieren ist fachlich reversibel und kein physisches Löschen.
...
**Funktion:** `search_knowledge`. **Input:** Anfrage, Typfilter, Limit und Mindestähnlichkeit. Unterstützte Typen: `note`, `task`, `list`, `list_item`, **seit W02 (2026-09-12) zusätzlich `fact`** (nur `claim_type='fact' AND status='active'`; `disputed`/`superseded`/`retracted` bleiben ausgeschlossen — siehe 20.3). **Output:** Hydrierte Wissensobjekte mit Suchscores, Suchstufen und Metadaten; keine generierte Antwort.
...
4. `build_messages` lädt Runtime-Kontext, den bisherigen Turn-Verlauf dieser Client-Conversation (seit W03, 2026-09-12) und hybride Wissenstreffer.
...
- **Behoben (W03, 2026-09-12):** Bis zu diesem Fix stammte die jüngste Konversation für jeden `ask_llm`-Aufruf ausschließlich aus `events` mit nichtleerer `response`, maximal 10 Events und höchstens 60 Minuten alt — ohne Bezug zur aktuellen Client-Conversation-ID. Da der Client-Chat-Pfad seine synthetischen `events`-Zeilen nie mit einer `response` versieht (das tut nur `/api/message`), traf dieser Filter für Client-Chat-Turns **nie** zu: eine Client-Conversation hatte dadurch de facto überhaupt kein Gedächtnis an ihre eigene Vorgeschichte, jeder Turn wurde faktisch wie der erste beantwortet. Live reproduziert: eine zweite Nachricht „Welchen Code habe ich dir gerade genannt?" wurde mit „Das ist deine erste Nachricht an mich" beantwortet, obwohl ein Code im vorherigen Turn derselben Conversation genannt worden war.
  Behoben durch `chat.py::get_conversation_turn_context()`, das stattdessen `client_conversation_messages` gezielt nach `conversation_id` filtert (dieselbe Tabelle, aus der `list_messages`/`run_chat_turn_once` ohnehin schon lesen/schreiben). `ask_llm`/`build_messages` nehmen dafür ein optionales `conversation_id`/`before_sequence`; ohne diese Parameter (der `/api/message`-Pfad in `routers/events.py`) bleibt `get_recent_conversation()` unverändert im Einsatz — das legale einzelne Legacy-Gespräch war von diesem Bug nicht betroffen. Live mit echten LLM-Aufrufen verifiziert: eine Folgefrage in derselben Conversation nennt jetzt korrekt den zuvor genannten Code; derselbe Ablauf gegen den unreparierten Code liefert reproduzierbar „kein Gedächtnis". Regressionstest: `m8_chat_conversation_memory_test.py`.
- **Behoben (W02, 2026-09-12):** Die allgemeine Suche umfasste keine Fact-Claims. Eine zu einem Fact promovierte und archivierte Note verschwand dadurch aus dem allgemeinen Chat-Retrieval, obwohl der Fact im Offline-Sync und im Referenzresolver verfügbar blieb — live bestätigt: eine Chat-Antwort auf eine Frage, deren einzige Quelle ein bereits promoveter Fact war, wusste vor dem Fix nichts davon. `search_knowledge` kennt jetzt den Typ `fact` (nur `status='active'`) über alle vier Hybrid-Kanäle (exakt/Vektor/FTS/Trigramm) sowie den Cache-Invalidierungs-Hash; `chat.py::build_messages` formatiert Treffer als `[Fact N]`. Bewusst ausgeschlossen bleiben `disputed`/`superseded`/`retracted` — es gibt noch keine Chat-seitige Behandlung für "umstrittenes Wissen anzeigen" (das ist W04s weiterhin offene Antworten-Guard-Hälfte, die durch diesen Fix erst technisch möglich wird, aber nicht automatisch mitgeliefert ist). `m8_fact_search_test.py`, inklusive eines echten LLM-Aufrufs, der einen Fact tatsächlich verwendet.
...
**Output:** Dokumentierter Konflikt, nicht automatisch eine korrigierte Wahrheit. Ein neuerer Satz gewinnt nicht pauschal. Die Konfliktprüfung ist nachts eingebunden und separat verfügbar; **seit W04 (2026-09-12) zusätzlich synchron ausgelöst**, sobald ein neuer aktiver Claim entsteht (`claims.py::create_claim_record` bzw. `materialize_validated_artifact_claims` bei tatsächlich neu angelegten, nicht-idempotenten Claims) — siehe 20.3. Das macht den bereits bestehenden `disputed`-Check der Note-zu-Fact-Promotion (11.3) tagesaktuell statt bis zu 24h veraltet. Kein Best-effort-Fehlschlag blockiert die auslösende Mutation; misslingt der eager Scan (z. B. Embedding-Endpoint down), bleibt der nächtliche Lauf als Rückfallebene unverändert bestehen. Weiterhin **kein** vorgeschalteter Schutz vor Chat-Antworten — dafür müsste `search_knowledge` (W02) überhaupt erst Claims kennen, was bislang nicht der Fall ist.
...
### 15.1 Die Kalendergrenze wurde durch eine echte Nachholgrenze ersetzt (W07, 2026-09-12)

`consolidate_today` wählte bis zu diesem Fix **den Tag des Aufrufs ab 00:00**, nicht den vorherigen Tag und nicht alle unverarbeiteten Events. Beim regulären Lauf um 03:00 erfasste es also nur Events zwischen 00:00 und 03:00 des neuen Tages; Events des vorangegangenen Nachmittags lagen außerhalb dieses Fensters und wurden nach einem ausgefallenen oder übersprungenen Lauf **nie wieder** erfasst, weil kein späteres Zeitfenster sie je wieder als „heute" einschließt.

Behoben durch `consolidation.py::get_pending_consolidation_events()` (vormals `get_today_unarchived_events()`): `events.archived = FALSE` ist bereits der alleinige, verlässliche Marker für „noch nicht konsolidiert" — genau das setzt auch der manuelle Endpoint `/api/events/{id}/unarchive` voraus. Die künstliche Untergrenze „Tagesbeginn" entfällt ersatzlos; die einzige verbleibende Grenze ist eine Obergrenze `as_of` (Default: Aufrufzeitpunkt), damit ein Lauf einen reproduzierbaren Stichtag statt einer impliziten Wall-Clock-Abhängigkeit hat. Ein ausgefallener Tag wird dadurch beim nächsten Lauf automatisch nachgeholt, in einem Rutsch, ohne separate Deckelung — live gegen die Produktions-DB verifiziert: ein realer, bereits bestehender Rückstand von 88 unverarbeiteten Events (8.–12.9., Ursache: eben dieser Bug) wurde in einem einzigen `consolidate_today()`-Aufruf vollständig verarbeitet und archiviert.

Der Konsolidierungs-Prompt (`CONSOLIDATION_SYSTEM_PROMPT`) verlangte bereits vorher, relative Zeitangaben pro Event relativ zum jeweiligen Event-Zeitstempel aufzulösen — ein mehrtägiger Rückstand in einem Lauf war also schon vorher inhaltlich korrekt handhabbar, nur die Auswahl der Events war zu eng. Der Prompt-Text wurde um einen Satz ergänzt, der das jetzt auch benennt, statt weiter „Events eines Tages" zu unterstellen.

Zusätzlich war `maintenance.py::run_daily_maintenance()` bis zu diesem Fix ein einziger sequenzieller Block ohne Fehlerisolation: eine Exception in einem frühen Schritt (z. B. `consolidate_today()`) ließ **alle** nachfolgenden Schritte der Nacht ausfallen, einschließlich der A11/W06-Nachtreparaturen. Jeder Schritt läuft jetzt einzeln isoliert (Fehler wird über `emit_event("maintenance", f"{step}_failed", "error", ...)` mit reinem `error_type` geloggt, nie mit Inhalt); das Ergebnis trägt zusätzlich `failed_steps`, damit ein Teilausfall nicht hinter einem unauffälligen Default verschwindet. Regressionstest: `m8_maintenance_error_isolation_test.py` (verifiziert gegen den unreparierten Code fehlschlagend, gegen den Fix grün; jetzt Teil von `m8_release_gate_test.py`s `TESTS`-Liste — ebenso nachträglich ergänzt: `m8_stt_uncertainty_test.py`, das dort bisher trotz gegenteiliger früherer Notiz fehlte).
...
| A10 | Capture-/Chat-Verarbeitung automatisch betreiben — **seit Re-Audit 2026-09-08 geschlossen, automatische Retries seit 2026-09-12 ergänzt:** Beide Queues sind Bestandteil von `worker.py all`; die manuellen Run-once-Endpunkte bleiben für Diagnose/Tests erhalten. Automatische Retries bereits fehlgeschlagener Capture-/Chat-Datensätze sind jetzt Teil von A11/W06 (siehe dort). | Persistierte Aufträge → ohne manuelle run-once-Aufrufe abgearbeitete Ergebnisse. |
| A11 | Idempotente Mehrfachaktionen und Recovery — **vollständig geschlossen (2026-09-12):** Ein Aktionsdatensatz pro Intentteil, Transaktion pro Mutation und Wiederaufnahme fehlgeschlagener Schritte verhindern die doppelte Ausführung bereits abgeschlossener Geschwister (A07, seit 2026-09-11). `client_sessions.attention_required` aus `capture_intent_failed`/`knowledge_finalization_failed`/`clarification_answer_failed` hatte zuvor überhaupt keinen Wiederholungsmechanismus — anders als `processing_jobs.parked` blieb eine so hängengebliebene Session für immer stehen, bis jemand manuell eingriff (live beobachtet an Session 1119); `client_sessions.py::retry_attention_required_client_sessions_for_night_repair` holt genau einmal je Session nach (seit 2026-09-11). Die beiden zuletzt offenen Fälle sind seit 2026-09-12 ebenfalls geschlossen: ein in `client_conversation_turns.status='failed'` hängengebliebener Chat-Turn erholte sich bisher nur, wenn der Client selbst den Retry-Endpunkt aufrief — `client_chat.py::queue_failed_chat_turns_for_night_repair` reiht ihn stattdessen genau einmal wieder in die bestehende Chat-Warteschlange ein, die `worker.py`s vorhandener `chat`-Worker unverändert abarbeitet. Und `promote_session_artifacts()` fängt einen Promotionsfehler je Artefakt intern ab (`session_artifacts.promotion_error`) und wirft ihn nie weiter — die zugehörige `client_session` erreicht trotzdem `completed`, unsichtbar für deren eigene Nachtreparatur, die nur den Sessionzustand prüft, nicht einzelne Artefakte; `promotion.py::claim_promotion_error_artifacts_for_night_repair`/`retry_promotion_errors_for_night_repair` holen das je Artefakt genau einmal nach. Alle drei Mechanismen teilen dieselbe Form: Einmal-Gate über `night_repair_attempts` (0..1, Migrationen `0050_client_session_night_repair`/`0051_chat_and_promotion_night_repair`), eingehängt in `run_daily_maintenance`. Neuer Test `m8_chat_promotion_night_repair_test.py` (im M8-Gate). **Vierter Fall seit 2026-09-12:** `client_sessions.state='processing'` mit vollständig `done`-gelaufenem Job-Graph, aber ohne Finalisierung — live an Session 1390 beobachtet: `settle_client_session_for_ingestion()` schluckt `ClientSessionConflict` laut eigenem Docstring bewusst still (Design-Annahme: „etwas anderes versucht es später erneut"), aber nichts tat das je, da ein wiederholtes `finish` bei bereits `processing`/`attention_required` No-op bleibt. Manueller erneuter Aufruf von `finalize_client_session_with_knowledge` schloss die reale Session sofort und fehlerfrei ab — die ursprüngliche Blockade war transient. `client_sessions.py::claim_stalled_processing_client_sessions_for_night_repair`/`retry_stalled_processing_client_sessions_for_night_repair` holen `processing`-Sessions mit vollständigem Job-Graph nach mindestens zehn Minuten Stillstand einmal nach; bleibt es dabei stehen, wird auf `attention_required` mit zurückgesetztem `night_repair_attempts` eskaliert — ein echter Übergang an die bestehende A11-Reparatur für einen zweiten, unabhängigen Versuch statt einer zweiten stillen Sackgasse. Neuer Test `m8_stalled_processing_night_repair_test.py`. | Retry / Absturz → Fortsetzung ab offenem Teilschritt ohne doppelte Mutation; liegengebliebene `attention_required`-Sessions, fehlgeschlagene Chat-Turns, promotionsfehlerhafte Artefakte und in `processing` mit fertigem Job-Graph steckengebliebene Sessions bekommen je einen nächtlichen Wiederholungsversuch. |
...
| W06 | Offene Vorgänge nachts nachholen — **vollständig geschlossen (2026-09-12):** ein nächtlicher Wiederholungsversuch je liegengebliebener `client_sessions.attention_required`-Session (Migration `0050_client_session_night_repair`, seit 2026-09-11), je fehlgeschlagenem Chat-Turn und je promotionsfehlerhaftem Artefakt (Migration `0051_chat_and_promotion_night_repair`, seit 2026-09-12) sowie, seit 2026-09-12, je in `processing` mit fertigem Job-Graph steckengebliebener Session, jeweils begrenzt auf genau einmal wie beim bestehenden Processing-Job-Nachholer. Details unter A11. | Zurückgestellte Kandidaten und Fehler → erneute Prüfung mit gespeichertem Kontext. |
...
Diese Übersicht dokumentiert die Abweichungen, ohne ältere normative Dateien still umzuschreiben. „Codebefund“ beschreibt den lokalen Stand; „Entscheidungsbedarf“ markiert eine noch zu klärende Produkt-/Vertragsfrage. Beim Re-Audit vom 2026-09-08 wurden D01–D22 erneut gegen die genannten Funktionen und Aufrufstellen geprüft: D02 und D18 sind geschlossen, D11 ist durch A10 kleiner geworden, alle übrigen Befunde wurden im aktuellen Code erneut bestätigt. Seither zusätzlich geschlossen: D08 (W02) und D09 (W03), beide 2026-09-12.
...
| D08 | Allgemeiner „Wissensspeicher“ vollständig für Antworten zugänglich | **Geschlossen (W02, 2026-09-12):** `search_knowledge` unterstützt jetzt `fact` (nur `status='active'`), siehe 9.1/10.2/20.3. | War: potenzieller Verlust der Chat-Auffindbarkeit nach Note-Archivierung. Live mit echtem LLM-Aufruf verifiziert. |
| D09 | Client-Chat als vollständig eigener Gesprächskontext | **Geschlossen (W03, 2026-09-12):** Promptkontext liest jetzt `client_conversation_messages` gezielt nach `conversation_id`, siehe 10.1/10.2/20.3. | War: Folgekontext-Lücke, Client-Chat hatte de facto kein Gedächtnis an die eigene Konversation. Live mit echten LLM-Aufrufen verifiziert. |
...
| A11 | Geschlossen | A07 besitzt einen eindeutigen Aktionsdatensatz je Intentteil, atomare Einzelmutation und Wiederaufnahme fehlgeschlagener Schritte; bereits abgeschlossene Geschwister werden nicht wiederholt. `client_sessions.attention_required` bekommt seit 2026-09-11 genau einen nächtlichen Wiederholungsversuch über `retry_attention_required_client_sessions_for_night_repair` (Migration `0050_client_session_night_repair`). Seit 2026-09-12 zusätzlich: `client_chat.py::queue_failed_chat_turns_for_night_repair` requeued fehlgeschlagene Chat-Turns genau einmal, `promotion.py::claim_promotion_error_artifacts_for_night_repair`/`retry_promotion_errors_for_night_repair` holen promotionsfehlerhafte Artefakte genau einmal nach (Migration `0051_chat_and_promotion_night_repair`). Seit 2026-09-12 zusätzlich: `claim_stalled_processing_client_sessions_for_night_repair`/`retry_stalled_processing_client_sessions_for_night_repair` holen `processing`-Sessions mit fertigem, aber nie finalisiertem Job-Graph nach und eskalieren bei erneutem Fehlschlag auf `attention_required`. |
...
| W02 | Geschlossen (2026-09-12) | `retrieval.py::search_knowledge` unterstützt jetzt zusätzlich `fact` (`claim_type='fact' AND status='active'`) über alle vier Hybrid-Kanäle, siehe 9.1/10.2. `m8_fact_search_test.py`, inkl. einem echten LLM-Aufruf, der einen zuvor unauffindbaren Fact tatsächlich in der Antwort verwendet. |
| W03 | Geschlossen (2026-09-12) | `ask_llm`/`build_messages` lesen den Turn-Verlauf einer Client-Conversation jetzt gezielt über `chat.py::get_conversation_turn_context()` aus `client_conversation_messages`, siehe 10.2. Der Legacy-Pfad `/api/message` (kein `conversation_id`) bleibt unverändert auf `get_recent_conversation()`. Live mit echten LLM-Aufrufen verifiziert (Vorher/Nachher-Vergleich); `m8_chat_conversation_memory_test.py`. |
| W04 | Teilweise geschlossen (Mutations-Hälfte, 2026-09-12) | `claims.py::create_claim_record`/`materialize_validated_artifact_claims` lösen `run_changed_conflict_scan` jetzt synchron aus (`_trigger_eager_conflict_scan`, best-effort), siehe 11.4. Der Antworten-Guard bleibt offen: `search_knowledge` kennt `disputed`/`superseded`/`retracted`-Claims seit W02 zwar technisch (sie werden bewusst herausgefiltert), aber es gibt noch keine Chat-seitige Behandlung, die einen umstrittenen Fact stattdessen sichtbar kennzeichnet statt ihn schlicht zu verstecken — eigene Umsetzungsentscheidung nötig. `m8_claim_conflict_guard_test.py`. |
...
| W06 | Geschlossen | `jobs.py::queue_parked_jobs_for_night_repair` deckt Processing-Jobs ab; seit 2026-09-11 holt `client_sessions.py::retry_attention_required_client_sessions_for_night_repair` zusätzlich liegengebliebene `client_sessions.attention_required`-Sessions nach (capture_intent_failed/knowledge_finalization_failed/clarification_answer_failed). Seit 2026-09-12 holen `client_chat.py::queue_failed_chat_turns_for_night_repair` fehlgeschlagene Chat-Turns und `promotion.py::retry_promotion_errors_for_night_repair` promotionsfehlerhafte `session_artifacts` (unabhängig vom Zustand ihrer Client-Session) je genau einmal nach — alle drei eingehängt in `run_daily_maintenance`. |
| W07 | Geschlossen (2026-09-12) | `consolidation.py::get_pending_consolidation_events` (vormals `get_today_unarchived_events`) hat keine Tagesuntergrenze mehr, nur noch eine Obergrenze `as_of`; `archived=FALSE` ist die alleinige Nachhol-Markierung, siehe 15.1. `maintenance.py::run_daily_maintenance` isoliert jeden Schritt einzeln (`failed_steps` im Ergebnis, Fehler nur mit `error_type` geloggt). Live gegen Produktion verifiziert: ein realer 88-Event-Rückstand (8.–12.9., durch genau diesen Bug entstanden) wurde in einem Lauf vollständig nachgeholt. `m8_maintenance_error_isolation_test.py`. |
