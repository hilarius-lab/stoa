...
| A11 | Idempotente Mehrfachaktionen und Recovery — **für A07-Objektaktionen und `client_sessions.attention_required` geschlossen (2026-09-11):** Ein Aktionsdatensatz pro Intentteil, Transaktion pro Mutation und Wiederaufnahme fehlgeschlagener Schritte verhindern die doppelte Ausführung bereits abgeschlossener Geschwister. Neu: `client_sessions.attention_required` aus `capture_intent_failed`/`knowledge_finalization_failed`/`clarification_answer_failed` hatte zuvor überhaupt keinen Wiederholungsmechanismus — anders als `processing_jobs.parked` blieb eine so hängengebliebene Session für immer stehen, bis jemand manuell eingriff (live beobachtet an Session 1119). `client_sessions.py::retry_attention_required_client_sessions_for_night_repair` holt genau einmal je Session nach, mit demselben Einmal-Gate (`night_repair_attempts`) wie `jobs.py::queue_parked_jobs_for_night_repair`, eingehängt in `run_daily_maintenance`. Allgemeine Chat-/Promotionsretries bleiben offen. | Retry / Absturz → Fortsetzung ab offenem Teilschritt ohne doppelte Mutation; liegengebliebene `attention_required`-Sessions bekommen genau einen nächtlichen Wiederholungsversuch. |
...
| W06 | Offene Vorgänge nachts nachholen — **für liegengebliebene `client_sessions.attention_required` seit 2026-09-11 teilweise geschlossen:** ein nächtlicher Wiederholungsversuch je Session (Migration `0050_client_session_night_repair`), begrenzt auf genau einmal wie beim bestehenden Processing-Job-Nachholer. Chat-Fehler und eine von der Client-Session unabhängige Promotionswiederholung bleiben offen. | Zurückgestellte Kandidaten und Fehler → erneute Prüfung mit gespeichertem Kontext. |
...
| A11 | Teilweise geschlossen | A07 besitzt einen eindeutigen Aktionsdatensatz je Intentteil, atomare Einzelmutation und Wiederaufnahme fehlgeschlagener Schritte; bereits abgeschlossene Geschwister werden nicht wiederholt. `client_sessions.attention_required` bekommt seit 2026-09-11 genau einen nächtlichen Wiederholungsversuch über `retry_attention_required_client_sessions_for_night_repair` (Migration `0050_client_session_night_repair`). Ein allgemeiner Nacht-/Retrymechanismus für Chat und Promotion fehlt weiter. |
...
| W06 | Teilweise geschlossen | `jobs.py::queue_parked_jobs_for_night_repair` deckt Processing-Jobs ab; seit 2026-09-11 holt `client_sessions.py::retry_attention_required_client_sessions_for_night_repair` zusätzlich liegengebliebene `client_sessions.attention_required`-Sessions nach (capture_intent_failed/knowledge_finalization_failed/clarification_answer_failed). Chat-Fehler und eine von der Client-Session unabhängige allgemeine Promotionswiederholung besitzen weiterhin keinen gemeinsamen Nacht-Nachholer. |
