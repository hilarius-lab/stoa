...
def main():
    ...
    try:
        ...
        _save_classification(artifact_id,{"candidate_type":"task","alternative_type":"note",
            "normalized_data":{"due_at":"2026-08-28T15:00:00+02:00","due_source":"explicit_relative"},
        ...
        assert data["due_at"]=="2026-08-28T15:00:00+02:00" and data["urgency"]==0.9,data
        ...
        with get_db_connection() as c:
            row=c.execute("SELECT due_at,urgency,urgency_source FROM tasks WHERE id=%s",(task_id,)).fetchone()
        assert row[0] is not None and row[1]==0.9 and row[2]=="explicit",row
